...
extern unsigned int autoloot;
extern unsigned int autoloot_on;

extern unsigned long long alootAddr;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		if(LOWORD(wParam)==5)
		{
			ReadProcessMemory(phandle, (LPVOID)(alootAddr + 0x1D30), &autoloot, sizeof(autoloot), 0);
			if(startbot==0)
			{
			...
			else if(autoloot == 0 && stopbot == 0)
			{
				WriteProcessMemory(phandle, (LPVOID)(alootAddr + 0x1D30), &autoloot_on, sizeof(autoloot_on), 0);
				SendMessage(activitylog, LB_ADDSTRING, NULL, (LPARAM)"BUGTEST!");
				updateList();
			}
			else if(startbot==2)
			{
