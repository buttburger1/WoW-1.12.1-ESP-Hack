...
DWORD mycodecave;
//unsigned int mycodecave;
//static const unsigned int mycodecave;// = 0x0041b200;
...
float pathuptime;// = clock() / (CLOCKS_PER_SEC / 1000);
float pathcurtime; //= clock() / (CLOCKS_PER_SEC / 1000);
float pathdeltatime;// = pathcurtime - pathuptime;
...
//function_t pfTheFunc = (function_t)0x704CD0;
...
//unsigned char hello[] = "RegisterCVar("testBBB","222"); DEFAULT_CHAT_FRAME:AddMessage("TestingBBB", 1.0, 0.0, 0.0);
...
//unsigned char hello[] = "HandleTerrainClick()";//NOWORK
...
void __declspec(naked) __start()
{
	__asm
	{
		...
		//mov edx, AllocatedMemoryCodeCave
...
void InjectModule()
{
	try
	{
		...
		*/
		...
		//string luatest = "CastSpellByName(\"Flash Heal(Rank 2)\");";
		//string luatest = "SendChatMessage(\"DoEmote(\"cry\")";
		//luatest.append(d);
		//"SendChatMessage(\"Hello World\")";
		//"DoEmote(\"cry\")");

		//std::cout << "string: " << luatest << " length:" << utf8_strlen(luatest) << endl;
		//luatestsize = utf8_strlen(luatest)+1;
		luatestsize = 1000;
		//printf("LUASIZE: %d\n", luatestsize);
		//printf("TEST: %c\n", luatest[0]);
		//printf("LUA - 0x%X\n", &luatest[0]);
		//DWORD m_dwIP = std::strtoul(str.c_str(), NULL, 16);
		//DWORD m_dwIP = std::strtoul(luatest.c_str(), NULL, 16);

		/*
		unsigned int num[64];
		for(int i = 0; i <= luatestsize; i++)
		{
			num[i] = luatest[i];
			printf("0x%X\n", num[i]);
		}
		*/
		...
		//AllocatedMemory = VirtualAllocEx(ProcessHandle, nullptr, (DWORD)__end - (DWORD)__start, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		//AllocatedMemoryCodeCave = VirtualAllocEx(ProcessHandle, nullptr, luatestsize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		...
		//printf("CODECAVE: 0x%X\n", AllocatedMemoryCodeCave);
		
		//mycodecave = (DWORD)AllocatedMemoryCodeCave;
		//printf("MYCODECAVE: 0x%X\n", mycodecave);
		//mycodecave = (unsigned int)0x4C1FA0;
		//mycodecave = (DWORD)0x4C1FA0;
		//printf("MYCODETEST: 0x%X\n", mycodecave);
		
		char * cstr = new char [luatest.length()+1];
		std::strcpy (cstr, luatest.c_str());

		//WriteProcessMemory(phandle, AllocatedMemoryCodeCave, shellcode, luatestsize, nullptr);//(DWORD)__end - (DWORD)__start

		//WriteProcessMemory(phandle, AllocatedMemoryCodeCave, hello, luatestsize, nullptr);//(DWORD)__end - (DWORD)__start
		WriteProcessMemory(phandle, AllocatedMemoryCodeCave, hello123, luatestsize, nullptr);//(DWORD)__end - (DWORD)__start

		//WriteProcessMemory(ProcessHandle, AllocatedMemoryCodeCave, hello, luatestsize, nullptr);//(DWORD)__end - (DWORD)__start
		//WriteProcessMemory(ProcessHandle, AllocatedMemoryCodeCave, cstr, luatestsize, nullptr);//(DWORD)__end - (DWORD)__start
		WriteProcessMemory(phandle, AllocatedMemory, &__start, (DWORD)__end - (DWORD)__start, nullptr);//(DWORD)__end - (DWORD)__start
		
		//CustomMemory = &AllocatedMemory + 0x1000;
		//char *p = static_cast<char*>(AllocatedMemory) - 8;
		//CustomMemory = static_cast<char*>(AllocatedMemoryCodeCave);//- 0x10000;
		//printf("TEST: 0x%X\n", CustomMemory);
		
		//CreateRemoteThread(ProcessHandle, nullptr, 0, (LPTHREAD_START_ROUTINE)AllocatedMemory, NULL, 0, nullptr);
		CreateRemoteThread(phandle, nullptr, 0, (LPTHREAD_START_ROUTINE)AllocatedMemory, AllocatedMemoryCodeCave, 0, nullptr);
		//CreateRemoteThread(ProcessHandle, nullptr, 0, (LPTHREAD_START_ROUTINE)AllocatedMemoryCodeCave, AllocatedMemory, 0, nullptr);
		//CreateRemoteThread(ProcessHandle, nullptr, 0, (LPTHREAD_START_ROUTINE)AllocatedMemoryCodeCave, NULL, 0, nullptr);
...
void objmanager()
{
	...
	//ReadProcessMemory(phandle, (LPVOID)0x00C4D890, &ctmPathX, sizeof(ctmPathX), 0);
	//ReadProcessMemory(phandle, (LPVOID)0x00C4D894, &ctmPathY, sizeof(ctmPathY), 0);
	//ReadProcessMemory(phandle, (LPVOID)0x00C4D898, &ctmPathZ, sizeof(ctmPathZ), 0);
	//ReadProcessMemory(phandle, (LPVOID)0x00C4D888, &ctmFlag, sizeof(ctmFlag), 0);
	...
	while(firstobj != 0 && (firstobj & 1) == 0)
	{
		...
		if(currentguid == localguid)
		{
			...
			//TEST CHECK TIME DIFFERENCE!
			/*
			time(&end_t);
			diff_t = difftime(end_t, start_t);
			if(diff_t > 3)tempbreak = 0;
			*/
			...
			else if(autoloot == 0 && tempbreak == 1 && isCasting == 0 && adjustedDeltaTime >= 250)
			{
				...
				if(numTargets != 0 && scans.targethp != 0)
				{
					...
					//tempResult = atan2(myy - scanY, myx - scanX);//DONOTUSE
					//tempResult = atan2(targety - myy, targetx - myx);
					//tempResult = atan2(npc.npcY[nextTarget] - scans.myy, npc.npcX[nextTarget] - scans.myx);
					...
					//FINAL RESULTS(0.1 for quick turn and cast, 0.9 or higher for melee)
					if(scans.melee == 0)
					{
					...
					{
						adjustedPosX = scans.myx + (param1 * dTemp * 0.9);
						adjustedPosY = scans.myy + (param2 * dTemp * 0.9);
					}
					...
					/*
					else if(targetrange > 1.0f && adjustedCTMdelta >= 1000)
					{
				...
				/*
				//III GOES TO 1ST AND JJJ SKIPS TO 2ND
				//if(scans.lineIndex[iii] == jjj)//&& breakloop == 0)//jjj++;
				//else if(breakloop == 1)jjj++;
				if(breakloop == 1)
				{
					jjj++;
				}
				...
				{
					//jjj++;
					strcpy(hello123, aaa[jjj]);//LOAD UP DOSTRING
				}
				*/
			...
			else if(autoloot == 1 && tempbreak == 1)
			{
				...
				/*NOT IN USE
				//FILLING IN CTM
				//WriteProcessMemory(phandle, (LPVOID)0x00C4D890, &scans.myx, sizeof(scans.myx), 0);
				//WriteProcessMemory(phandle, (LPVOID)0x00C4D894, &scans.myy, sizeof(scans.myy), 0);
				//WriteProcessMemory(phandle, (LPVOID)0x00C4D898, &scans.myz, sizeof(scans.myz), 0);
				
				//WRITE TO AUTOLOOT TO HALT PATHING
				//cReadTextFile(fDirectory);
				//tempbreak = 2;
								
				strcpy(hello123, aaa[jjj]);
				pathcurtime = clock() / (CLOCKS_PER_SEC / 1000);
				pathdeltatime = pathcurtime - pathuptime;

				if(jjj == totalpaths)tempbreak = 2;
				else if(pathdeltatime >= 250)//500ms
				{
					pathuptime = clock() / (CLOCKS_PER_SEC / 1000);
					InjectModule();
				...
				//NO USE, WILL STALL PROGRAM
				do
				{
					strcpy(hello123, aaa[jjj]);
					pathcurtime = clock() / (CLOCKS_PER_SEC / 1000);
					pathdeltatime = pathcurtime - pathuptime;
					if(pathdeltatime >= 3000)
					{
						InjectModule();
						jjj++;
					}
				} while(jjj < totalpaths);

				for(int i = 0; i < totalpaths; i++)//totalpaths
				{
					strcpy(hello123, aaa[i]);
					//MessageBox(hwnd, hello123, "TEST123", MB_OK);
					Sleep(500);
					InjectModule();
				}
				...
				//float radiusF;
				//distanceR(myx, myy, myz, ctmPathX[0], ctmPathY[0], ctmPathZ[0], radiusF);
				//float radiusF = (float)sqrt((ctmPathX[0]*myx) + (ctmPathY[0]*myy) + (ctmPathZ[0]*myz));
				//if(radiusF < 1.0f)
				//{
					//SIMPLE PATHING SCRIPT
					//WriteProcessMemory(phandle, (LPVOID)0x00C4D890, &ctmPathX[0], sizeof(ctmPathX[0]), 0);
					//WriteProcessMemory(phandle, (LPVOID)0x00C4D894, &ctmPathY[0], sizeof(ctmPathY[0]), 0);
					//WriteProcessMemory(phandle, (LPVOID)0x00C4D898, &ctmPathZ[0], sizeof(ctmPathZ[0]), 0);
					//WriteProcessMemory(phandle, (LPVOID)0x00C4D888, &ctmFlag4, sizeof(ctmFlag4), 0);
				//}
				*/
			}
			...
			if(startPath == 1 && totalTargets != 0 && tempbreak != 0 && adjustedNewPosDelta >= 2000)//if(startPath == 0)
			{
				if(adjustedBreak == 0)
				{
					...
					//adjustedBreak = 1;
					//GET CURRENT POS
					//scans.myPrevX = scans.myx, scans.myPrevY = scans.myy, scans.myPrevZ = scans.myz;
					//adjustedBreak = 1;
				}
			}
			
			/*
			if(startPath == 1 && totalTargets == 0 && tempbreak == 0 && adjustedNewPosDelta >= 250)//totalTargets,tempbreak
			{
				adjustedNewPosTime = clock() / (CLOCKS_PER_SEC / 1000);
				//scans.changedPos = distanceR(scans.myx, scans.myPrevX, scans.myy, scans.myPrevY, scans.myz, scans.myPrevZ);
				scans.changedPos = distanceR(scans.myPrevX, scans.myx, scans.myPrevY, scans.myy, scans.myPrevZ, scans.myz);

				//if(scans.changedPos < 1.0f)//MOVE TO THE NEXT POINT
				//if(scans.changedPos == scans.prevPos)//MOVE TO THE NEXT POINT
				scans.deltaPos = scans.changedPos - scans.prevPos;
			if(scans.deltaPos <= 1.0f)
			{
				WriteProcessMemory(phandle, (LPVOID)(firstobj + 0x1D30), &autoloot_on, sizeof(autoloot_on), 0);
			}
				scans.prevPos = scans.changedPos;
			}
			*/
	...
	*/
	...
	if(isingame == 1)
	{
		...
		if(breaker == 0)
		{
			...
			//MY STATS
			//myhp,mymana,mycurrentmana,myrage
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpvReserved)
{
	...
	///WaitForInputIdle(phandle, 10000);
